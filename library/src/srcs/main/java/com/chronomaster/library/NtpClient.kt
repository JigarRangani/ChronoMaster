...
object NtpClient {
     ...
     * @return A [ChronoResult] containing the current time in milliseconds since the Unix epoch on success,
     *         or an error message if the request fails.
     */
    suspend fun getTrueTime(ntpPool: String = DEFAULT_NTP_POOL): ChronoResult<Long> = withContext(Dispatchers.IO) {
        ...
        return@withContext try {
            ...
            val trueTime = currentTime + offset

            ChronoResult.Success(trueTime)
        } catch (e: Exception) {
